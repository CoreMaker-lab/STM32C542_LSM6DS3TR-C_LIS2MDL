...
//	  platform_init();
	  ...
	  if ((lsm6ds3tr_c_pin_mode_set(&dev_ctx, LSM6DS3TR_C_PUSH_PULL) != 0) ||
	      (lsm6ds3tr_c_pin_polarity_set(&dev_ctx, LSM6DS3TR_C_ACTIVE_LOW) != 0) ||
	      (lsm6ds3tr_c_data_ready_mode_set(&dev_ctx,
	                                       LSM6DS3TR_C_DRDY_LATCHED) != 0))
	  {
	      printf("LSM6DS3TR-C INT1 configuration error\r\n");
	      while (1);
	  }
	  pin_int.int1_drdy_xl = PROPERTY_ENABLE;
	  pin_int.int1_drdy_g = PROPERTY_ENABLE;
	  if (lsm6ds3tr_c_pin_int1_route_set(&dev_ctx, pin_int) != 0)
	  {
	      printf("LSM6DS3TR-C INT1 routing error\r\n");
	      while (1);
	  }
	  /* Service any data already pending when INT1 was enabled. */
	  thread_wake = 1U;
    while (1) {
        if (thread_wake != 0U)
        {
            ...
            thread_wake = 0U;
            lsm6ds3tr_c_status_reg_t status = {0};
            int32_t ret;

            /* Read output only if new data is available. */
            ret = lsm6ds3tr_c_status_reg_get(&dev_ctx, &status);

            if ((ret == 0) && (status.xlda != 0U))
            {
                memset(data_raw_acceleration, 0x00, 3 * sizeof(int16_t));
                ret = lsm6ds3tr_c_acceleration_raw_get(&dev_ctx,
                                                       data_raw_acceleration);
            }
            if ((ret == 0) && (status.gda != 0U))
            {
                memset(data_raw_angular_rate, 0x00, 3 * sizeof(int16_t));
                ret = lsm6ds3tr_c_angular_rate_raw_get(&dev_ctx,
                                                       data_raw_angular_rate);
            }
            if ((ret == 0) && (status.tda != 0U))
            {
                memset(&data_raw_temperature, 0x00, sizeof(int16_t));
                ret = lsm6ds3tr_c_temperature_raw_get(&dev_ctx,
                                                      &data_raw_temperature);
            }

            if (ret != 0)
            {
                printf("LSM6DS3TR-C read error, retrying\r\n");
                platform_delay(100);
                thread_wake = 1U;
                continue;
            }
            ...
            {
                temperature_degC =
                    lsm6ds3tr_c_from_lsb_to_celsius(data_raw_temperature);
                printf("Temperature [degC]:%6.2f\r\n",
                       (double)temperature_degC);
            }

            /* XL and GY share INT1. Check pending data before waiting
             * for another edge, including a source that became ready late.
             */
            ret = lsm6ds3tr_c_status_reg_get(&dev_ctx, &status);
            if (ret != 0)
            {
